...
import { getActiveMcpName, getClinician } from "../session.js";
...
router.post("/chat/start", requireCorti, async (req, res) => {
  ...
  try {
    const warmup: Corti.AgentsMessage = {
      role: "user",
    ...
    if (!contextId) {
      res.status(502).json({ error: "Could not obtain a context id from Corti." });
      return;
    }
    ...
    if (!bindResponse.ok) {
      ...
      res.status(502).json({ error: body.error || `Pre-bind failed: ${bindResponse.status}` });
      return;
    }
...
router.post("/agents/:id/message", requireCorti, async (req, res) => {
  try {
    ...
    if (clinician) {
      parts.push({
        kind: "data",
        data: { type: "token", mcp_name: getActiveMcpName(), token: mintScopeToken(clinician) },
      });
    }
