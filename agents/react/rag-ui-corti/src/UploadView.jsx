...
export default function UploadView({ clinician, onBack }) {
  ...
  const scopeLabel = (scopeValue) =>
    scopeValue === SHARED
      ? "Shared / reference (all clinicians)"
      : scopeValue.replace(/^patient:/, "");
        ...
        <div className="max-w-2xl mx-auto">
          {uploadResult ? (
            <Banner variant="success" className="p-4">
              ...
              <p className="text-xs">
                Scope: <span style={MONO}>{scopeLabel(uploadResult.scope)}</span>
              </p>
              ...
              <div>
                ...
                >
                  {patients.map((patient) => (
                    <option key={patient.mrn} value={`patient:${patient.mrn}`}>
