import { AlertTriangle, Loader2, LogOut, Sparkles } from "lucide-react";
import { useEffect, useState } from "react";
import { getActiveAgent, getSetupConfig, provisionAgent } from "./api.js";
import { Banner, MONO, ScreenHeader } from "./ui.jsx";
      ...
      <main className="flex-1 px-4 py-8">
        <div className="max-w-2xl mx-auto space-y-4">
          <p className="text-sm text-muted-foreground">
            No orchestrator wired to this document search service was found in your tenant. Review
            what it will be created with, give it a name, and confirm.
          </p>
          ...
          {config && !config.mcpConfigured && (
            <Banner variant="warning" className="flex items-start gap-2">
              ...
              <span>
                MCP_URL is not configured on the server. Set it in the app's environment before
                creating the agent.
              </span>
            </Banner>
          )}
          ...
          <div>
            ...
            <pre
              className="bg-card border border-border rounded-lg p-3 text-xs text-foreground whitespace-pre-wrap leading-relaxed"
              style={MONO}
            >
              {config?.systemPrompt}
            </pre>
