...
export const signInClinician = (clinicianId) =>
  request("/api/clinician/login", {
    method: "POST",
    body: { clinicianId },
    label: "Clinician sign-in",
  });

// ── Agent setup ────────────────────────────────────────────────────────────────
export const getSetupConfig = () =>
  request("/api/agent/setup-config", { label: "Get setup config" });
...
export const getAgentTask = (agentId, taskId) =>
  request(`/api/agents/${encodeURIComponent(agentId)}/task/${encodeURIComponent(taskId)}`, {
    label: "Get task",
  });
