...
export function TextReplacements() {
  ...
  const config = useMemo(() => buildReplacementConfig(LANGUAGE, items), [items]);

  return (
    <div className="flex flex-col gap-4">
      <div>
        <h2 className="text-lg font-semibold text-foreground">Text replacements</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Replacements rewrite spoken phrases in the final transcript (e.g. “BID” → “twice daily”).
          Configure the rule set below, then dictate to see them applied. {items.length} rules
          configured.
        </p>
      </div>
