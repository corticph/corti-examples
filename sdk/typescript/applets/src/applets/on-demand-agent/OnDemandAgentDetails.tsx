    ...
    <div className="flex flex-col gap-4 text-sm">
      ...
      <div>
        ...
        <ul className="ml-4 mt-1 list-disc text-muted-foreground">
          <li>
            On first use the agent is created via{" "}
            <code>client.agents.create</code> if one named “
            {COPY_EDIT_AGENT.name}” doesn’t already exist; its id is cached per
            API client.
          </li>
          <li>
            A copy-edit sends the editor text via{" "}
            <code>client.agents.messageSend</code> (no contextId — each call is
            isolated) and replaces the editor with the returned text. When the
            agent makes no edits it returns the text unchanged, so your content
            is always preserved.
          </li>
          <li>
            Editing the prompt calls <code>client.agents.update</code> so the
            behavior changes without recreating the agent.
          </li>
        </ul>
      </div>

      <p className="rounded-md bg-muted/60 px-2 py-1.5 text-xs text-muted-foreground">
        Requires an auth token scoped for the Corti{" "}
        <strong>Agentic Framework</strong> for the API client credentials. See
