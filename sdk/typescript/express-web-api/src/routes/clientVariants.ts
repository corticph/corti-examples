...
async function clientVariants(req: Request, res: Response): Promise<void> {
  ...
  async function run(
    ...
    try {
      ...
      results.push({ name, status: "error", message: e instanceof Error ? e.message : String(e), ...opts });
    }
  }

  // 1. CC — explicit tenant + env (string)
  if (cc) {
    await run("cc-explicit", () => new CortiClient({
  ...
  if (ropc) {
    await run("ropc-explicit", () => new CortiClient({
  ...
  if (cc) {
    await run("cc-env-library", () => new CortiClient({
      tenantName: cc.tenantName,
  ...
  if (cc) {
    await run("cc-env-custom-urls", () => new CortiClient({
      tenantName: cc.tenantName,
      environment: {
  ...
  //    header is sent.
  if (cc) {
    await run("env-custom-urls-no-auth", () => new CortiClient({
      environment: {
        base: `https://api.${cc.environment}.corti.app/v2`,
        wss: `wss://api.${cc.environment}.corti.app/audio-bridge/v2`,
        login: `https://auth.${cc.environment}.corti.app/realms`,
  ...
  if (cc) {
    ...
    const token = await cortiAuth.getToken({ clientId: cc.clientId, clientSecret: cc.clientSecret });
    const accessToken = token.accessToken ?? "";

    // 6. Bearer — explicit tenant + env
    await run("bearer-explicit", () => new CortiClient({
      tenantName: cc.tenantName,
      environment: cc.environment,
      auth: { accessToken },
    }));
    ...
    await run("bearer-auto-derive", () => new CortiClient({
      auth: { accessToken },
    }));

    // 8. Bearer — explicit + custom refreshAccessToken (accessToken is primary; fn called on expiry)
    await run("bearer-with-refresh-fn", () => new CortiClient({
      tenantName: cc.tenantName,
      environment: cc.environment,
      auth: {
        accessToken,
        refreshAccessToken: () => cortiAuth.getToken({ clientId: cc.clientId, clientSecret: cc.clientSecret }),
      },
    }));

    // 9. Bearer — custom refreshAccessToken as primary, seeded with initial accessToken (avoids an immediate refresh call)
    await run("bearer-custom-refresh-seeded", () => new CortiClient({
      tenantName: cc.tenantName,
      environment: cc.environment,
      auth: {
        refreshAccessToken: () => cortiAuth.getToken({ clientId: cc.clientId, clientSecret: cc.clientSecret }),
    ...
    await run("baseUrl-cc", () => new CortiClient({
      tenantName: cc.tenantName,
  ...
  if (ropc) {
    ...
    const getRopcToken = (): ReturnType<CortiAuth["getRopcFlowToken"]> =>
      cortiAuth.getRopcFlowToken({ clientId: ropc.clientId, username: ropc.username, password: ropc.password });

    // 11. refreshAccessToken only — explicit tenant + env
    await run("refresh-fn-explicit", () => new CortiClient({
      tenantName: ropc.tenantName,
      environment: ropc.environment,
    ...
    await run("refresh-fn-auto-derive", () => new CortiClient({
      auth: { refreshAccessToken: getRopcToken },
    }));
    ...
    if (ropcToken.refreshToken) {
      await run("bearer-with-builtin-refresh", () => new CortiClient({
        tenantName: ropc.tenantName,
        environment: ropc.environment,
        auth: {
          ...
          ...(ropcToken.refreshExpiresIn != null ? { refreshExpiresIn: ropcToken.refreshExpiresIn } : {}),
        },
      }));
    } else {
      results.push({
