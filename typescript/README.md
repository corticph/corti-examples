...
2. **Create Environment File**
    - In the `typescript/` directory, create a file named `.env.local`.
    - Copy the contents of `.env.example` and fill in the required values.

3. **Install Dependencies** (from the `typescript/` directory)
   ```bash
...
- **Examples** (all paths relative to this `typescript/` directory):
